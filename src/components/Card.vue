<template>
  <li class="relative flex flex-col gap-2 h-full">
    <div
      ...
      class="card rounded-4 w-120px h-220px sm:(w-180px) lg:(w-220px h-320px) 2xl:(w-320px h-420px) opacity-60"
    />

    <!-- <img :src="`/zb-website/cards/${i}.webp`" height="220" width=""> -->
    <span class="pl-1 whitespace-pre flex flex-col">
      <span class="text-base font-thin text-#787878 2xl:text-lg">
        {{ parsedDate ? parsedDate : date.extra }}
      </span>
      <span v-if="date.hide" class="text-sm text-#A9A8A8 text-xl 2xl:text-3xl"> ??? </span>
